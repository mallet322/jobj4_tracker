package ru.job4j.lambda.folder;

import java.util.List;
import java.util.function.Predicate;

import org.hamcrest.Matchers;
import org.junit.Assert;
import org.junit.Test;

public class SearchFolderTest {

    @Test
    public void whenFilterSize() {
        List<Folder> list = List.of(
                new Folder("fix", 110),
                new Folder("bug", 75),
                new Folder("bug", 90)
        );
        List<Folder> expected = List.of(new Folder("fix", 110));
        Predicate<Folder> pred = f -> f.getSize() > 100;
        List<Folder> rsl = SearchFolder.filter(list, pred);
        Assert.assertThat(rsl, Matchers.is(expected));
    }

    @Test
    public void whenFilterName() {
        List<Folder> list = List.of(
                new Folder("fix", 110),
                new Folder("bug", 75),
                new Folder("bug", 90)
        );
        List<Folder> expected = List.of(
                new Folder("bug", 75),
                new Folder("bug", 90)
        );
        Predicate<Folder> pred = f -> f.getName().contains("bug");
        List<Folder> rsl = SearchFolder.filter(list, pred);
        Assert.assertThat(rsl, Matchers.is(expected));
    }

    @Test
    public void whenFilterSizeAndName() {
        List<Folder> list = List.of(
                new Folder("fix", 110),
                new Folder("bug", 75),
                new Folder("bug", 90)
        );
        List<Folder> expected = List.of(
                new Folder("bug", 75)
        );
        Predicate<Folder> predSize = f -> f.getSize() < 90;
        Predicate<Folder> predName = f -> f.getName().contains("bug");
        List<Folder> rsl = SearchFolder.filter(list, predSize.and(predName));
        Assert.assertThat(rsl, Matchers.is(expected));
    }

}